...
export default class PrintServiceNewTaxImpl implements PrintService {
  ...
  public constructor(currencyFormatterService: CurrencyFormatterServiceImpl,
                     dateFormatterService: DateFormatterServiceImpl,
  ...
  printReceipt(saleData: Sale, paying: number) {
    console.log("USING NEW PRINT SERVICE");
    ...
    const taxes = this.taxService.calculateTaxesSaleResponse(saleData.taxes, saleData.subTotal)
    ...
    let company = companyName + newLine;
    ...
    const GRAND_TOTAL = `Grand Total:${fourSpaceTab.repeat(4 - fourSpaceTabRepeat)}<strong>${grandTotal}</strong>${newLine}`;
    const SUB_TOTAL = `Sub Total:${fourSpaceTab.repeat(5 - fourSpaceTabRepeat)}${subTotal}${newLine}`;

    let TAX_PERCENTAGE = '';
    for (const tax of taxes) {
      const taxValue = this.currencyFormatterService.format(tax!.value);
      let repp = 0
      if (tax.name === 'NHIL') {
        repp = 10
      } else if (tax.name === 'VAT') {
        repp = 13
      } else if (tax.name === 'Get-Fund') {
        repp = 4
      } else if (tax.name === 'Covid') {
        repp = 13
      }

      TAX_PERCENTAGE += `${tax?.name} ${tax.percent}%${singleSpace.repeat(repp)}${taxValue}`;
      // TAX_PERCENTAGE += this.spaceOutStrings(30, `${tax?.name} ${tax.percent}%`, taxValue);
      TAX_PERCENTAGE += newLine;
    }
  ...
  spaceOutStrings(pageWidth: number, str1: string, str2: string): string {
    const totalWidth = str1.length + str2.length;
    const remainingSpace = pageWidth - totalWidth;

    if (remainingSpace >= 0) {
      // Calculate the number of spaces to insert between the strings
      const spacesBetweenStrings = singleSpace.repeat(Math.floor(remainingSpace / 2));
      console.log('spacesBetweenStrings', spacesBetweenStrings)

      // Return the spaced-out strings
      return str1 + spacesBetweenStrings + str2;
    } else {
      // If the total width of the strings is greater than the page width, return the original strings
      return str1 + str2;
    }
  }
